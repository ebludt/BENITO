...
void setup(){
    Serial.begin(9600);
    Serial.println( "Mando unificado V.0.1.7" );
   pinMode(JoyPinRB, INPUT_PULLUP);
...
void loop(){ 
  ...
  case 2:
   ...
   RBbuttonState = digitalRead(JoyPinRB);
    if (RBbuttonState==LOW){ //pulsado
        if (pulsadoRB==1) pulsadoRB=0;
        else pulsadoRB=1;
    }
    
    if (pulsadoRB==1) Vals[2]=map(analogRead(JoyPinRY),0,1024,255,0);
